...
function Kraken() {
	...
	function* positionsClose(Command, position, market) {
		Command.p.relative(position.price)

		Command.q.reference(position.vol)
		...
		if (Command.t === "market") {
			...
			params.price = Command.p.resolve(market.pair_decimals)
		}
		...
		if (Command.d) {
			console.info(this.getExchangeName(), params, "validated order", order)
		...
		if (Command.cm.getMax() < positions.length) {
			switch (Command.cmo) {
				...
				case "highest":
					...
					break
				case "smallest":
					sortByIndex(positions, "vol")
					break
				case "biggest":
					sortByIndex(positions, "vol", true)
					break
				case "random":
					shuffle(positions)
			}
	...
	function* positionsOpen(symbol) {
		...
		for (let txid in positions) {
			if (positions.hasOwnProperty(txid)) {
				...
				position.leverage = Math.floor(position.cost / position.margin)
